...
def _get_calendar():
    ...
    return (calendars[0], True) if calendars else (None, True)
...
def calendar_tool(command: str) -> str:
    ...
    """
    # Trigger calendar animation on display
    try:
        from common import show_tool_animation_sync

        show_tool_animation_sync("calendar", {})
    except Exception:
        pass

    calendar, is_configured = _get_calendar()
    ...
    task_create = re.search(
        r"(?:add|create)\s+(?:a\s+)?task\s+(?:called\s+)?(.+)", command_lower
    )
    if task_create:
...
END:VCALENDAR"""
    ...
    if any(
        ...
        for word in ["left", "to do", "to-do", "pending", "what else"]
    ):
...
END:VCALENDAR"""
    ...
    if any(
        ...
        for phrase in ["what's on", "what is on", "my calendar", "upcoming"]
    ):
