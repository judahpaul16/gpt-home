...
def lights_tool(command: str) -> str:
    ...
    """
    # Trigger lights animation on display
    try:
        ...
        show_tool_animation_sync("lights", {"action": action})
    except Exception:
        pass
    ...
    color_match = re.search(
        r"\b(red|green|blue|yellow|purple|orange|pink|white)\b", command_lower
    )
    if color_match:
    ...
    if brightness_match and any(
        word in command_lower for word in ["dim", "bright", "brightness"]
    ):
        brightness_pct = int(brightness_match.group(1))
        brightness = max(0, min(254, int(brightness_pct * 254 / 100)))
        bridge.set_group(0, "on", True)
