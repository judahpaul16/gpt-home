...
async def _get_coords_from_city(
    city: str, api_key: Optional[str] = None
) -> Optional[dict]:
...
async def weather_tool(query: str) -> str:
    ...
    city_match = re.search(
        r"(?:in|for|at)\s+([\w\s,]+?)(?:\?|$|today|tomorrow|this|next)",
        query,
        re.IGNORECASE,
    )
    ...
    if not city:
        ...
        if not city:
            return (
                ...
                "You can set a default location in settings, or ask like "
    ...
    if "current" in data:
        ...
        is_day = bool(data["current_weather"].get("is_day", 1))
