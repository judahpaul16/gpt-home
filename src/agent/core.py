...
from src.tools import ToolRegistry, get_all_tools
...
class GPTHomeAgent(BaseAgent):
    ...
    async def initialize(self):
        ...
        if not api_key:
            raise ValueError(
                "LITELLM_API_KEY is not configured. Please add your API key in Settings > Integrations."
            )
        ...
        if self.store and state.get("user_id"):
            try:
                ...
                if store:
                    ...
                    if memories:
                        memories_text = "\n".join(
                            [f"- {m.value.get('content', m.value)}" for m in memories]
                        )
            except Exception:
                pass
...
Be concise but helpful in your responses as they will be spoken aloud."""
    ...
    async def stream(
        ...
        config: RunnableConfig = {
            ...
            # Limit recursion to prevent infinite tool call loops
            "recursion_limit": 15,
        }
