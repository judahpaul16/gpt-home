...
const AppContent: React.FC = () => {
    ...
    const setStatus = useCallback((name: string, status: boolean) => {
        setIntegrations((prev) => {
            // Optimization: Don't trigger a re-render if value is the same
            if (prev[name as keyof typeof prev]?.status === status) {
                return prev;
            }
            return {
                ...prev,
