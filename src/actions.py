...
from src.tools.weather import WEATHER_CODES as weather_codes
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(
                f"http://{ip}/spotify-control", json={"text": text}
            )
            if response.status == 200:
                ...
                try:
                    data = await response.json()
                    msg = data.get("message") or await response.text()
                except Exception:
                    msg = await response.text()
                logger.warning(msg)
                return f"Received a {response.status} status code. {msg}"
...
async def city_from_zip(zip_code: str, country_code: str = "us"):
    async with aiohttp.ClientSession() as session:
        try:
            ...
            response = await session.get(url, headers={"User-Agent": "gpt-home"})
            if response.status == 200:
                json_response = await response.json()
                if json_response:
                    first = json_response[0]
                    # Try to extract a reasonable city/locality name
                    city = first.get("display_name", "").split(",")[0]
                    return city or None
                else:
                    logger.error("No results from Nominatim for postal code")
            else:
                logger.error(
                    f"Failed to retrieve city from postal code. Status: {response.status}"
                )
        except Exception:
            logger.error(
                f"Error retrieving city from zip code: {traceback.format_exc()}"
            )
    return None
...
async def open_weather_action(text: str):
    try:
        ...
        def _get_openweather_api_key_from_db():
            try:
                import json
                import os
                ...
                logger.debug(f"OpenWeather key lookup failed: {e}")
            return None

        api_key = _get_openweather_api_key_from_db()
        ...
        async with aiohttp.ClientSession() as session:
            if re.search(r"(weather|temperature).*\sin\s", text, re.IGNORECASE):
                ...
                if not re.search(r"(forecast|future)", text, re.IGNORECASE):
                    ...
                    if api_key:
                        response = await session.get(
                            f"https://api.openweathermap.org/data/3.0/onecall?lat={coords.get('lat')}&lon={coords.get('lon')}&appid={api_key}&units=imperial"
                        )
                        if response.status == 200:
                            json_response = await response.json()
                            logger.debug(json_response)
                            weather = (
                                json_response.get("current")
                                .get("weather")[0]
                                .get("main")
                            )
                    ...
                    response = await session.get(
                        f"https://api.open-meteo.com/v1/forecast?latitude={coords.get('lat')}&longitude={coords.get('lon')}&current_weather=true&temperature_unit=fahrenheit"
                    )
                    if response.status == 200:
                        json_response = await response.json()
                        weather_code = json_response.get("current_weather").get(
                            "weathercode"
                        )
                        temp = json_response.get("current_weather").get("temperature")
                        weather_description = (
                            weather_codes[str(weather_code)]["day"]["description"]
                            if datetime.now().hour < 18
                            else weather_codes[str(weather_code)]["night"][
                                "description"
                            ]
                        )
                ...
                # Weather forecast
                    ...
                    if api_key:
                        ...
                        if response.status == 200:
                            ...
                            tomorrow_forecast = list(
                                filter(
                                    lambda x: x.get("date") == tomorrow.strftime("%A"),
                                    forecast,
                                )
                            )[0]
                            speech_responses = []
                            speech_responses.append(
                                f"Tomorrow, it will be {tomorrow_forecast.get('temp')}\u00b0F and {tomorrow_forecast.get('weather')} in {city}."
                            )
                            for day in forecast:
                                if day.get("date") != tomorrow.strftime("%A"):
                                    speech_responses.append(
                                        f"On {day.get('date')}, it will be {round(float(day.get('temp')))} degrees and {day.get('weather').lower()} in {city}."
                                    )
                    ...
                    # Fallback to Open-Meteo
                    response = await session.get(
                        f"https://api.open-meteo.com/v1/forecast?latitude={coords.get('lat')}&longitude={coords.get('lon')}&daily=temperature_2m_max,temperature_2m_min&temperature_unit=fahrenheit"
                    )
                    if response.status == 200:
                        json_response = await response.json()
                        forecast = []
                        for day in json_response.get("daily"):
                            ...
                            day_weather_description = (
                                weather_codes[str(day_weather_code)]["day"][
                                    "description"
                                ]
                                if datetime.now().hour < 18
                                else weather_codes[str(day_weather_code)]["night"][
                                    "description"
                                ]
                            )
                            forecast.append(
                        ...
                        speech_responses = []
                        speech_responses.append(
                            f"Tomorrow, it will be between {tomorrow_forecast.get('temp_min')}\u00b0F and {tomorrow_forecast.get('temp_max')}\u00b0F and {tomorrow_forecast.get('weather_description').lower()} in {city}."
                        )
                        for day in forecast:
                            if day.get("date") != tomorrow.strftime("%Y-%m-%d"):
                                speech_responses.append(
                                    f"On {day.get('date')}, it will be between {day.get('temp_min')}\u00b0F and {day.get('temp_max')}\u00b0F and {day.get('weather_description').lower()} in {city}."
                                )
                        combined_response = " ".join(speech_responses)
                        return await llm_action(
                            text=f"""Provide a concise response to the user's question based on the weather data.  Do not summarize or respond to anything other than the question\n
                ...
                if api_key:
                    response = await session.get(
                        f"http://api.openweathermap.org/data/3.0/onecall?lat={coords.get('lat')}&lon={coords.get('lon')}&appid={api_key}&units=imperial"
                    )
                    if response.status == 200:
                        json_response = await response.json()
                        logger.debug(json_response)
                        weather = (
                            json_response.get("current").get("weather")[0].get("main")
                        )
                        temp = json_response.get("current").get("temp")
                ...
                if response.status == 200:
                    ...
                    weather_description = (
                        weather_codes[str(weather_code)]["day"]["description"]
                        if datetime.now().hour < 18
                        else weather_codes[str(weather_code)]["night"]["description"]
                    )
                    combined_response = f"It is currently {round(float(temp))} degrees and {weather_description.lower()} in {city}."
                    return await llm_action(
                        text=f"""Provide a concise response to the user's question based on the weather data.  Do not summarize or respond to anything other than the question\n
        ...
        raise Exception(
            "No Open Weather API key found. Please enter your API key for Open Weather in the web interface or try reconnecting the service."
        )
...
async def philips_hue_action(text: str):
    ...
    if bridge_ip and username:
        try:
            ...
            # Change light color
            color_pattern = (
                r"\b(red|green|blue|yellow|purple|orange|pink|white|black)\b"
            )
    ...
    raise Exception(
        "No philips hue bridge IP found. Please enter your bridge IP for Phillips Hue in the web interface or try reconnecting the service."
    )
        ...
        try:
            response = completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are a helpful assistant. {settings.get('custom_instructions')}",
                    },
...
def set_alarm(command, minute, hour, day_of_month, month, day_of_week, comment):
    ...
    alarm_time = now.replace(
        minute=minute, hour=hour, day=day_of_month, month=month, second=0, microsecond=0
    )
...
async def reminder(text, delay):
    time.sleep(delay)
    await speak(text)


def set_reminder(
    text, minute, hour, day_of_month, month, day_of_week=None, comment=None
):
    now = datetime.now()
    ...
    asyncio.run(reminder(text, delay))
...
async def alarm_reminder_action(text):
    ...
    if set_match:
        ...
        minute, hour, dom, month, dow = parse_time_expression(time_expression)
        comment = "Reminder"
        return set_reminder(reminder_text, minute, hour, dom, month, dow, comment)
    else:
...
async def caldav_action(text: str):
    ...
    if not url or not username or not password:
        return (
            "CalDAV server credentials are not properly set in environment variables."
        )
    ...
    try:
        ...
        task_create_match = re.search(
            r"\b(?:add|create)\s+a?\s+task\s+called\s+(.+)", text, re.IGNORECASE
        )
        task_delete_match = re.search(
            r"\b(?:delete|remove)\s+(a )?task\s+called\s+(\w+)", text, re.IGNORECASE
        )
        task_update_match = re.search(
            r"\b(?:update|change|modify)\s+(a )?task\s+called\s+(\w+)\s+to\s+(\w+)",
            text,
            re.IGNORECASE,
        )
        tasks_query_match = re.search(
            r"\b(left|to do|to-do|what else)\b", text, re.IGNORECASE
        )
        completed_tasks_query_match = re.search(
            r"\bcompleted\s+tasks\b", text, re.IGNORECASE
        )
...
""")
            ...
            for task in tasks:
                if task_name.lower() in task.instance.vtodo.summary.value.lower():
                    ...
                    task.save()
                    return (
                        f"Task '{task_name}' updated to '{new_task_name}' successfully."
                    )
        ...
        create_match = re.search(
            r"\b(?:add|create|schedule)\s+an?\s+(event|appointment)\s+called\s+(\w+)\s+on\s+(\d{4}-\d{2}-\d{2})\s+at\s+(\d{1,2}:\d{2})",
            text,
            re.IGNORECASE,
        )
        update_match = re.search(
            r"\b(?:update|change|modify)\s+the\s+(event|appointment)\s+called\s+(\w+)\s+to\s+(\w+)\s+on\s+(\d{4}-\d{2}-\d{2})\s+at\s+(\d{1,2}:\d{2})",
            text,
            re.IGNORECASE,
        )
        delete_match = re.search(
            r"\b(?:delete|remove|cancel)\s+the\s+(event|appointment)\s+called\s+(\w+)",
            text,
            re.IGNORECASE,
        )
        next_event_match = re.search(
            r"\bwhat'? ?i?s\s+my\s+next\s+(event|appointment)\b", text, re.IGNORECASE
        )
        calendar_query_match = re.search(
            r"\bwhat'? ?i?s\s+on\s+my\s+calendar\b", text, re.IGNORECASE
        )

        if create_match:
            event_name = create_match.group(1)
            event_time = datetime.strptime(
                f"{create_match.group(2)} {create_match.group(3)}", "%Y-%m-%d %H:%M"
            )
            event_start = event_time.strftime("%Y%m%dT%H%M%S")
            event_end = (event_time + timedelta(hours=1)).strftime(
                "%Y%m%dT%H%M%S"
...
""")
            ...
            event_time = datetime.strptime(
                f"{update_match.group(4)} {update_match.group(5)}", "%Y-%m-%d %H:%M"
            )
            event_start = event_time.strftime("%Y%m%dT%H%M%S")
            event_end = (event_time + timedelta(hours=1)).strftime(
                "%Y%m%dT%H%M%S"
